...
| Enemy action / player damage | `BATTLE_ENEMY_ATTACK`, `BATTLE_PLAYER_DAMAGE` | SILENT | Dedicated assets are needed; player attack is not reused. |
| Enemy defeat / player defeat | `BATTLE_DEFEATED`, `BATTLE_GAME_OVER` | IMPLEMENTED | Defeat has one verified file, not five. |
| Escape success / failure | `BATTLE_ESCAPE_SUCCESS`, `BATTLE_ESCAPE_FAIL` | SILENT | Semantic hooks retained while assets are absent. |
